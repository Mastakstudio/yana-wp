...
if ( ! class_exists('FalconGlen_WooCommerce') ) :

    class FalconGlen_WooCommerce {
...
//            add_filter('woocommerce_add_to_cart_fragments', 'addToCartFragmentsByAjax');
//            add_filter('woocommerce_add_to_cart_fragments', 'addWoocommerceMiniCartAjax');

            // Удаляем фильтр, который проверяет, является ли количество целым числом
            remove_filter('woocommerce_stock_amount', 'intval');

            // Добавляем фильтр, который проверяет, является ли количество числом float
...
//            do_action( 'storefront_woocommerce_setup' );
        ...
        function addWoocommerceMiniCartAjax($fragments)
        {
        ...
        function addToCartFragmentsByAjax($fragments)
        {
            global $woocommerce;
            ob_start();
            ...
            ?>
            <span class="counter header-menu__counter cart-customlocation <?php echo ($count == 0) ? 'empty' : 'added not-empty'; ?>">
                <?php echo $count; ?>
            </span>
            <?php
...
return new FalconGlen_WooCommerce();